...
def preprocess_dataset(
    ...
    def format_example(examples): # support question with a single answer or multiple answers
        for i in range(len(examples["prompt"])):
            if examples["prompt"][i] and examples["response"][i]:
                ...
                #print(f"query: {query}")
                #print(f"answer: {answer}, examples[query][i]:", examples["query"][i])
    ...
    def preprocess_supervised_dataset(examples):
        ...
        for prompt, answer in format_example(examples):
            #print(f"preprocess_supervised_dataset prompt: {prompt}")
            #print(f"preprocess_supervised_dataset answer: {answer}")
            ...
            #print(f"source_ids 2: {source_ids}")
            #print(f"target_ids 2: {target_ids}")
            ...
            #print(f"IGNORE_INDEX: {IGNORE_INDEX}, context_length: {context_length}, input_ids[context_length:]: {input_ids[context_length:]}")
